...
const findInTree = (path: string[], key: string, arr: TreeNode[] = []) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (key && arr[i].key && arr[i].key === key) keyNode = keyNode || arr[i]

    if (arr[i].path.join('.') === path.join('.')) pathNode = pathNode || arr[i]
  }
...
const clone = (options: CloneOptions) => {
  ...
  const children: any = React.Children.map(options.children, (child: any) => {
    ...
    // CASE: Formable Node
    let path = [...options.path, name, nodeIndex && `[${nodeIndex}]`].filter(
      x => x
    )

...
export class TLeaf<T> {
    ...
    public sequence(): any {
        return Promise.resolve(this.value)
            ...
            .catch(err => TLeaf.of(err));
    }
}

export class TArray<T> {
    public value: T;
    public children: ChildrenArray<T>;
    ...
    public sequence(): any {
        const childrenPromises = this.children.map(child => child.sequence());
        const promises: Promise<Tree<T> | T>[] = [Promise.resolve(this.value), ...childrenPromises];
...
export class TObject<T> {
    public value: T;
    public children: ChildrenObject<T>;
    ...
    public sequence(): any {
        ...
        return promiseEvery([Promise.resolve(this.value), ..._values])
            .then(args => {
                const value = args[0];
